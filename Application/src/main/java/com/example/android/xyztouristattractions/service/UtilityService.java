...
public class UtilityService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ACTION_ADD_GEOFENCES.equals(action)) {
            ...
            String city = currentLocation == null ? TouristAttractions.TEST_CITY :
                    TouristAttractions.getClosestCity(currentLocation, getApplicationContext());
    ...
    private void showNotification(String cityId, boolean microApp) {
        ...
        List<Attraction> attractions = TouristAttractions.loadAttractionsFromLocation(Utils.getLocation(getApplicationContext()),getApplicationContext());
        ...
        try {
            for (int i = 0; i < count; i++) {
                bitmaps.put(attractions.get(i).getCityListData().getPlaceName(),
                        Glide.with(this)
                                .load(attractions.get(i).getCityListData().getCityImagePath())
                                .asBitmap()
        ...
        PendingIntent pendingIntent = PendingIntent.getActivity(this, 0,
                DetailActivity.getLaunchIntent(this, attraction.getCityListData().getPlaceCode()),
        ...
        builder = new NotificationCompat.Builder(this)
                .setStyle(new NotificationCompat.BigPictureStyle()
                        .bigPicture(bitmaps.get(attraction.getCityListData().getPlaceName()))
                        .setBigContentTitle(attraction.getCityListData().getCityIntro())
                        .setSummaryText(getString(R.string.nearby_attraction))
                )
                .setLocalOnly(microApp)
                .setContentTitle(attraction.getCityListData().getPlaceName())
        ...
        if (!microApp) {
            ...
            for (int i = 1; i < count; i++) {
                ...
                String distance = Utils.formatDistanceBetween(
                        Utils.getLocation(this), attractions.get(i).getCityListData().getLocation());

                // Construct the notification and add it as a page
                pages.add(new NotificationCompat.Builder(this)
                        .setContentTitle(attractions.get(i).getCityListData().getPlaceName())
                        .setContentText(distance)
                        .setSmallIcon(R.drawable.ic_stat_maps_pin_drop)
                        .extend(new NotificationCompat.WearableExtender()
                                .setBackground(bitmaps.get(attractions.get(i).getCityListData().getPlaceName()))
                        )
                        .build());
            }
    ...
    private void sendDataToWearable(List<Attraction> attractions) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (image != null && secondaryImage != null) {
                ...
                String distance = Utils.formatDistanceBetween(
                        Utils.getLocation(this), attraction.getCityListData().getLocation());

                attractionData.putString(Constants.EXTRA_TITLE, attraction.getCityListData().getPlaceName());
                attractionData.putString(Constants.EXTRA_DESCRIPTION, attraction.getCityListData().getCityIntro());
                attractionData.putDouble(
                        Constants.EXTRA_LOCATION_LAT, attraction.getCityListData().getLocation().latitude);
                attractionData.putDouble(
                        Constants.EXTRA_LOCATION_LNG, attraction.getCityListData().getLocation().longitude);
                attractionData.putString(Constants.EXTRA_DISTANCE, distance);
                attractionData.putString(Constants.EXTRA_CITY, attraction.getCityListData().getPlaceCode());
